apple.controller("tasktest", [
    ...
    function ($scope, $http, $filter, $routeParams, $location,) {
        ...
        gettasks = async function () {
            console.log('1', await GetData());
            debugger
            const response = await $http.get(source, {
                /*  headers: {
            ...
            if (lasthash != hash) {
                ...
                allTasks.forEach(task => {
                    task.TAGS = task.TAGS.filter(t => !t.includes("_"))
                })
                ...
                angular.forEach(response.data.tasks, function (tasks, uid) {
                    ...
                    angular.forEach(tasks, function (task, ndx) {
                        if (task.TAGS && task.TAGS.length) {
                            ...
                            const tags = task.TAGS.filter(t => !t.includes("_")||!t.includes("_ADT"))
                            const mytags = task.TAGS.filter(t => t.includes("_")||t.includes("_ADT"))
                            if (tags.length) {
